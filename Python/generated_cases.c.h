...
static py_tail_call_funcptr INSTRUCTION_TABLE[256] = {
    ...
    dispatch_opcode:
        ...
        TARGET(BINARY_OP) {
            ...
            DISPATCH();
            #ifdef Py_TAIL_CALL_INTERP
            tail_call_pop_4_error:
            TAIL_CALL(pop_4_error);

            tail_call_pop_3_error:
            TAIL_CALL(pop_3_error);

            tail_call_pop_2_error:
            TAIL_CALL(pop_2_error);

            tail_call_pop_1_error:
            TAIL_CALL(pop_1_error);

            tail_call_error:
            TAIL_CALL(error);

            tail_call_exception_unwind:
            TAIL_CALL(exception_unwind);

            tail_call_exit_unwind:
            TAIL_CALL(exit_unwind);

            tail_call_start_frame:
