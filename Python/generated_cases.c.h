    ...
    dispatch_opcode:
        ...
        TARGET(BINARY_OP_ADD_INT) {
            ...
            #endif
            {
                ...
                // _GUARD_TOS_INT
                {
                    value = stack_pointer[-1];
                    PyObject *value_o = PyStackRef_AsPyObjectBorrow(value);
                    if (Py_UNLIKELY(!_PyLong_CheckExactAndCompact(value_o))) {
                        UPDATE_MISS_STATS(BINARY_OP);
                        assert(_PyOpcode_Deopt[opcode] == (BINARY_OP));
                        JUMP_TO_PREDICTED(BINARY_OP);
                    }
                ...
                // _GUARD_NOS_INT
                {
                    left = stack_pointer[-2];
                    PyObject *left_o = PyStackRef_AsPyObjectBorrow(left);
                    if (Py_UNLIKELY(!_PyLong_CheckExactAndCompact(left_o))) {
                        UPDATE_MISS_STATS(BINARY_OP);
                        assert(_PyOpcode_Deopt[opcode] == (BINARY_OP));
                        JUMP_TO_PREDICTED(BINARY_OP);
                    }
        ...
        TARGET(FOR_ITER_LIST) {
            ...
            #endif
            {
                ...
                // _ITER_CHECK_LIST
                {
                    ...
                    if (Py_UNLIKELY(Py_TYPE(iter_o) != &PyList_Type)) {
                        UPDATE_MISS_STATS(FOR_ITER);
                        assert(_PyOpcode_Deopt[opcode] == (FOR_ITER));
                        JUMP_TO_PREDICTED(FOR_ITER);
                    }
                    ...
                    #ifdef Py_GIL_DISABLED
                    if (Py_UNLIKELY(!_Py_IsOwnedByCurrentThread(iter_o) && !_PyObject_GC_IS_SHARED(iter_o))) {
                        UPDATE_MISS_STATS(FOR_ITER);
                        assert(_PyOpcode_Deopt[opcode] == (FOR_ITER));
                        JUMP_TO_PREDICTED(FOR_ITER);
                    }
        ...
        TARGET(FOR_ITER_TUPLE) {
            ...
            #endif
            {
                ...
                // _ITER_CHECK_TUPLE
                {
                    ...
                    iter = stack_pointer[-2];
                    PyObject *iter_o = PyStackRef_AsPyObjectBorrow(iter);
                    if (Py_UNLIKELY(Py_TYPE(iter_o) != &PyTuple_Type)) {
                        UPDATE_MISS_STATS(FOR_ITER);
                        assert(_PyOpcode_Deopt[opcode] == (FOR_ITER));
                        JUMP_TO_PREDICTED(FOR_ITER);
                    }
                    ...
                    if (oparg & 1) {
                        *attr = PyStackRef_NULL;
                        ...
                        if (is_meth) {
                            ...
                            PyStackRef_CLOSE(owner);
                            stack_pointer = _PyFrame_GetStackPointer(frame);
                            if (Py_UNLIKELY(PyStackRef_IsNull(*attr))) {
                                JUMP_TO_LABEL(error);
                            }
        ...
        TARGET(LOAD_ATTR_GETATTRIBUTE_OVERRIDDEN) {
            ...
            #endif
            {
                ...
                assert(type_version != 0);
                if (Py_UNLIKELY(FT_ATOMIC_LOAD_UINT_RELAXED(cls->tp_version_tag) != type_version)) {
                    UPDATE_MISS_STATS(LOAD_ATTR);
                    assert(_PyOpcode_Deopt[opcode] == (LOAD_ATTR));
                    JUMP_TO_PREDICTED(LOAD_ATTR);
                }
                ...
                if (Py_UNLIKELY(f->func_version != func_version)) {
                    UPDATE_MISS_STATS(LOAD_ATTR);
                    assert(_PyOpcode_Deopt[opcode] == (LOAD_ATTR));
                    JUMP_TO_PREDICTED(LOAD_ATTR);
                }
